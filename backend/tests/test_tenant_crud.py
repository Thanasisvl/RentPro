...
def owner_headers_and_tenant_id():
    owner, headers = register_and_login(
        client,
        username="owner_tenant_1",
        password="testpassword",
        email="owner_tenant_1@example.com",
        is_owner=True,
    )
    resp = client.post(
        "/tenants/",
...
def test_create_tenant_authenticated():
    _, headers = register_and_login(
        client,
        username="owner_tenant_2",
        password="testpassword",
        email="owner_tenant_2@example.com",
        is_owner=True,
    )
    resp = client.post(
        "/tenants/",
...
def test_create_tenant_missing_fields(owner_headers_and_tenant_id):
    headers, _ = owner_headers_and_tenant_id
    resp = client.post("/tenants/", json={"name": "No AFM"}, headers=headers)
...
def test_create_tenant_invalid_afm():
    _, headers = register_and_login(
        client,
        username="owner_invalid_afm",
        password="testpassword",
        email="owner_invalid_afm@example.com",
        is_owner=True,
    )
    resp = client.post(
        "/tenants/",
...
def test_cross_owner_tenant_access_and_admin_access():
    ...
    _, owner1_headers = register_and_login(
        client, "owner_x1", "testpassword", "owner_x1@example.com", is_owner=True
    )
    t_resp = client.post(
        "/tenants/",
        json={
    ...
    _, owner2_headers = register_and_login(
        client, "owner_x2", "testpassword", "owner_x2@example.com", is_owner=True
    )
    assert client.get(f"/tenants/{tenant_id}", headers=owner2_headers).status_code in (
        403,
        404,
    )
    assert client.put(
        f"/tenants/{tenant_id}", json={"name": "Hacked"}, headers=owner2_headers
    ).status_code in (403, 404)
    assert client.delete(
        f"/tenants/{tenant_id}", headers=owner2_headers
    ).status_code in (403, 404)
