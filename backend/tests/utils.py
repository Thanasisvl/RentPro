...
def set_area_score(property_id: int, area_score: float | None):
    ...
    db = SessionLocal()
    try:
        row = db.query(PropertyLocationFeatures).filter(PropertyLocationFeatures.property_id == property_id).first()
        if row is None:
            row = PropertyLocationFeatures(property_id=property_id, area_score=area_score)
            db.add(row)
        else:
...
def set_pairwise_all_equal(client, headers):
    ...
    body = {
        "comparisons": [
            ...
            {"criterion_a_key": "price", "criterion_b_key": "property_type", "value": 1},
            ...
            {"criterion_a_key": "property_type", "criterion_b_key": "area_score", "value": 1},
        ]
    }
    resp = client.post("/preference-profiles/me/pairwise-comparisons", json=body, headers=headers)
    assert resp.status_code == 200
    return resp.json()
...
def create_property(client, headers, **overrides):
    ...
    if "type" in payload and payload["type"] is not None:
        ...
        assert payload["type"] in PROPERTY_TYPE_ALLOWED, f"Invalid test Property.type: {payload['type']}"
