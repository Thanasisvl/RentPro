...
def create_tenant(
    ...
    *,
    ...
    if exists:
        raise HTTPException(
            status_code=409, detail="Tenant with this AFM already exists for this owner"
        )
...
def list_tenants(
    db: Session, *, owner_id: int | None = None, skip: int = 0, limit: int = 100
):
    q = db.query(Tenant)
    ...
    *,
    ...
    if "afm" in data and data["afm"] != db_tenant.afm:
        ...
        if exists:
            raise HTTPException(
                status_code=409,
                detail="Tenant with this AFM already exists for this owner",
            )
